...
public class SafetyNetResponse {
     ...
     */
    @Nullable
    private static SafetyNetResponse parse(@NonNull String decodedJWTPayload, String jwsResult) {

        Log.d(TAG, "decodedJWTPayload json:" + decodedJWTPayload);

        SafetyNetResponse response = new SafetyNetResponse();

        response.jwsResult = jwsResult;
    ...
    public static SafetyNetResponse parseJsonWebSignature(String jwsResult) {
        ...
        if (jwtParts.length == 3) {
            ...
            return SafetyNetResponse.parse(decodedPayload, jwsResult);
        } else {
            return null;
        }
    }
